...
parser.add_argument(
    "--fi",
    type=int,
    help=("Number of Fan-in Switches to create." "Must be >= 1"),
    required=True,
)
...
parser.add_argument(
    "--n",
    type=int,
    help=("Number of hosts to create in each lower level switch." "Must be >= 1"),
    required=True,
)
...
class DataCenter(Topo):
    ...
    def __init__(self, n=1, delay="0ms", fi=1, cpu=0.01, max_queue_size=None, **params):
        ...
        for level in range(1, fi + 1):
            mls = self.addSwitch(f"mls{level}")
            self.addLink(tls, mls, **swlinkConfig)
            for switch_id in range(1, fi + 1):
                switch = self.addSwitch(f"s{level}x{switch_id}")
                self.addLink(mls, switch, **swlinkConfig)
                for host_id in range(1, n + 1):
                    host = self.addHost(f"h{level}x{switch_id}x{host_id}", **hostConfig)
                    self.addLink(switch, host, **hostlinkConfig)
