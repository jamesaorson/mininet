...
topo = FirewallTopo()
net = Mininet(
    topo=topo, link=TCLink, controller=RemoteController("SDNFirewall", port=6633)
)
net.start()
...
for i in range(len(testcases)):
    ...
    if protocol == "P":
        ...
        # TCP, UDP
        host_message = host.cmdPrint(
            "python test-server.py %s %s %s &" % (protocol, host_ip, port)
        )
        time.sleep(sleep_time)
        client_message = client.cmdPrint(
            "python test-client.py %s %s %s &" % (protocol, host_ip, port)
        )
        time.sleep(sleep_time)
    ...
    if connected != ans:
        wrong_ans.append(
            "%d: %s -> %s with %s at %s, should be %s, current %s"
            % (i, client_name, host_name, protocol, port, ans, connected)
        )
